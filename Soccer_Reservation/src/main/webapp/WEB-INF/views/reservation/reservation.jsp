		...
		<ul class="pagination">
			<c:choose>
				<c:when test="${startPageNum ne 1 }">
					<li><a
						href="reservation.do?pageNum=${startPageNum-1 }&condition=${condition}&keyword=${keyword}">&laquo;</a>
					</li>
				</c:when>
				<c:otherwise>
			...
			<c:forEach var="i" begin="${startPageNum }" end="${endPageNum }">
				<c:choose>
					<c:when test="${i eq pageNum }">
						<li class="active"><a
							href="reservation.do?pageNum=${i }&condition=${condition}&keyword=${keyword}">${i }</a></li>
					</c:when>
					<c:otherwise>
						<li><a
							href="reservation.do?pageNum=${i }&condition=${condition}&keyword=${keyword}">${i }</a></li>
			...
			<c:choose>
				<c:when test="${endPageNum lt totalPageCount }">
					<li><a
						href="reservation.do?pageNum=${endPageNum+1 }&condition=${condition}&keyword=${keyword}">&raquo;</a>
					</li>
				</c:when>
				<c:otherwise>
					...
					<div class="input-group">
						<div class="input-group-btn">
							...
							<ul class="dropdown-menu">
								<li><a href="javascript:set('rUser')">신청인</a></li>
								<li><a href="javascript:set('rAddr')">운동장</a></li>
								<li><a href="javascript:set('rTime')">대여시간</a></li>
							</ul>
						</div>
						<!-- /btn-group -->
